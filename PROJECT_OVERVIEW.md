...
6. **Actionable Empty States**: UI windows like Inventory, Quests, and Trade implement context-aware empty states with actionable hints rather than just showing a blank window when empty.
7. **Item Value Context**: Tooltips and descriptions dynamically surface item values to aid player decision making when trading or evaluating inventory.
