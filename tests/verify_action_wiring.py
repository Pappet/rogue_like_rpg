...
def make_stats(hp: int = 100, max_hp: int = 100) -> Stats:
    return Stats(hp=hp, max_hp=max_hp, power=5, defense=2, mana=50, max_mana=50, perception=10, intelligence=10)
...
def test_investigate_action_requires_targeting():
    ...
    assert investigate.range == 10, f"Expected range=10, got {investigate.range}"
...
def test_investigate_action_cost_is_zero():
    ...
    investigate = next((a for a in action_list.actions if a.name == "Investigate"), None)
    assert investigate is not None, "Investigate action not found in ActionList"
    assert investigate.cost_mana == 0, f"Expected cost_mana=0, got {investigate.cost_mana}"
...
def test_description_get_accepts_none_stats():
    ...
    assert result2 == "A portal", f"Expected base text 'A portal' when stats=None, got '{result2}'"
...
def test_description_get_still_works_with_stats():
    ...
    desc = Description(base="An orc", wounded_text="A wounded orc", wounded_threshold=0.5)

    # Healthy entity — returns base
    healthy_stats = make_stats(hp=100, max_hp=100)
    assert desc.get(healthy_stats) == "An orc", f"Expected 'An orc' for healthy stats, got '{desc.get(healthy_stats)}'"
    ...
    assert desc.get(zero_stats) == "An orc", f"Expected 'An orc' when max_hp=0, got '{desc.get(zero_stats)}'"
...
def test_confirm_action_skips_end_turn_for_inspect():
    ...
    player = esper.create_entity(
        Position(2, 2),
        make_stats(),
        ActionList(actions=[Action(name="Investigate", range=10, requires_targeting=True, targeting_mode="inspect")]),
    )
...
def test_render_system_cursor_colors():
    ...
    assert inspect_range == (0, 255, 255, 50), f"Expected cyan range color for inspect, got {inspect_range}"
    assert inspect_cursor == (0, 255, 255), f"Expected cyan cursor color for inspect, got {inspect_cursor}"

    # Combat/auto mode should produce yellow
    auto_range, auto_cursor = get_colors_for_mode("auto")
    assert auto_range == (255, 255, 0, 50), f"Expected yellow range color for auto, got {auto_range}"
    assert auto_cursor == (255, 255, 0), f"Expected yellow cursor color for auto, got {auto_cursor}"

    # Manual mode should also produce yellow (not inspect)
    manual_range, manual_cursor = get_colors_for_mode("manual")
    assert manual_range == (255, 255, 0, 50), f"Expected yellow range color for manual, got {manual_range}"
    assert manual_cursor == (255, 255, 0), f"Expected yellow cursor color for manual, got {manual_cursor}"

    # Verify actual render_system.py contains the cyan color constant
    with open(
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "game", "systems", "render_system.py")
    ) as f:
        source = f.read()
    assert "0, 255, 255" in source, "render_system.py does not contain cyan color (0, 255, 255)"
