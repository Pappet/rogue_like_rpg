...
def test_interaction_resolver():
    ...
    hostile = world.create_entity(
        AIBehaviorState(AIState.IDLE, Alignment.HOSTILE), Stats(10, 10, 5, 5, 5, 5, 1, 1), Name("Monster")
    )
    assert InteractionResolver.resolve(world, 0, hostile) == InteractionType.ATTACK

    # 2. Sleeping
    sleeper = world.create_entity(
        AIBehaviorState(AIState.SLEEP, Alignment.NEUTRAL), Stats(10, 10, 5, 5, 5, 5, 1, 1), Name("Villager")
    )
    assert InteractionResolver.resolve(world, 0, sleeper) == InteractionType.WAKE_UP

    # 3. Friendly
    friendly = world.create_entity(
        AIBehaviorState(AIState.IDLE, Alignment.FRIENDLY), Stats(10, 10, 5, 5, 5, 5, 1, 1), Name("Friend")
    )
    assert InteractionResolver.resolve(world, 0, friendly) == InteractionType.TALK

    # 4. Neutral
    neutral = world.create_entity(
        AIBehaviorState(AIState.IDLE, Alignment.NEUTRAL), Stats(10, 10, 5, 5, 5, 5, 1, 1), Name("NPC")
    )
