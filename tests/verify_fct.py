...
def test_fct():
    ...
    attacker = esper.create_entity(
        Name("Attacker"), Stats(hp=10, max_hp=10, power=5, defense=0, mana=0, max_mana=0, perception=5, intelligence=5)
    )
    ...
    ent, fct = fcts[0]
    print(
        f"FCT created: text={fct.text}, pos=({esper.component_for_entity(ent, Position).x}, {esper.component_for_entity(ent, Position).y})"
    )
