...
from game.components import Action, Description, ItemMaterial, Name, Portable, Position, Stats, Targeting
...
def test_inspection_output_detailed():
    ...
    player = world.create_entity(
        Position(0, 0),
        Stats(hp=10, max_hp=10, power=5, defense=2, mana=10, max_mana=10, perception=5, intelligence=5),
    )
    targeting_action = Action("Inspect", targeting_mode="inspect", range=5)

    # We need to set targeting component
    targeting = Targeting(
        origin_x=0, origin_y=0, target_x=0, target_y=0, range=5, mode="inspect", action=targeting_action
    )
