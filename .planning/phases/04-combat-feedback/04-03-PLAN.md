...
Output: Working combat system with feedback and lifecycle events.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    2.  Modify `ecs/systems/action_system.py`:
        -   In movement logic: If target tile has `Blocker`, check if target entity has `Fighter`.
...
<task type="auto">
  ...
  <action>
    1.  Create `ecs/systems/combat_system.py`:
        ...
        -   Get stats from `Fighter` components of attacker and target.
  ...
  <done>Combat loop functional and dispatches death events.</done>
</task>
