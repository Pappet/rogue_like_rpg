...
<task type="auto">
  ...
  <action>
    ...
    - Implement a "Targeting Mode" with two subtypes:
        - **Auto-Targeting**: Automatically cycle through entities with `Renderable` components that are in VISIBLE tiles (within LoS and range).
...
<task type="auto">
  ...
  <action>
    ...
    - Ensure successful actions advance the turn via `TurnSystem`.
  </action>
