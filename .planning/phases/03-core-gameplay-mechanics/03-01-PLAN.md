...
wave: 1
depends_on: []
files_modified: [main.py, game_states.py, services/party_service.py, ecs/components.py, ecs/world.py, ecs/systems/render_system.py, ecs/systems/movement_system.py]
autonomous: true
---
...
Output: A functional game engine running on ECS with basic movement and rendering.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    - Create `ecs/components.py` with the following components:
        - `Position(x, y)`
...
<task type="auto">
  <name>Task 2: Implement core ECS systems</name>
  <files>ecs/systems/render_system.py, ecs/systems/movement_system.py, game_states.py</files>
  <action>
    ...
    - Update `game_states.py` (`Game` class) to:
        ...
        - Add `RenderSystem` and `MovementSystem` to the world.
        - Process the world during `update` and `draw`.
  </action>
  <verify>The game runs and renders the player party using the new ECS systems.</verify>
  <done>Render and Movement systems are integrated into the game loop.</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    - Assign appropriate components (`Position`, `Renderable`, `Stats`, `TurnOrder`) to these entities.
...
<success_criteria>
The game is successfully refactored to use `esper` ECS for its core loop (rendering and movement).
</success_criteria>
