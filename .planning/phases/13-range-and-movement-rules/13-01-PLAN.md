...
must_haves:
  truths:
    - "Investigation cursor cannot move beyond a range equal to the player's perception stat"
...
Do NOT change `confirm_action()` — Phase 14 will update that method when adding inspection output for SHROUDED tiles.
...
3. `confirm_action()` is unchanged
  ...
  <done>
`start_targeting()` sets `targeting.range` to `stats.perception` when `targeting_mode == "inspect"`. `move_cursor()` allows movement to any tile whose visibility_state is not UNEXPLORED. All 7 existing Phase 12 tests still pass.
  </done>
</task>
