...
wave: 2
depends_on: [27-01]
files_modified: [ecs/components.py, game_states.py, ecs/systems/ui_system.py]
autonomous: true

must_haves:
  truths:
    - "Map transitions (portals) advance the clock by travel_ticks"
    - "TurnSystem.round_counter stays in sync with WorldClock.total_ticks"
...
Output: Updated Portal component, travel-time logic, and enhanced UI header.
</objective>
...
<task type="auto">
  ...
  <files>ecs/components.py, game_states.py</files>
  <action>
    1. Add `travel_ticks: int = 0` to `Portal` dataclass in `ecs/components.py`.
    2. In `game_states.py`, update `Game.transition_map` to:
       - Accept `travel_ticks` in `event_data` (default 0).
       - Call `world_clock.advance(travel_ticks)` before entering the new map.
