...
<task type="auto">
  ...
  <action>
    1. Add TOD constants to `config.py`:
       - `TICKS_PER_HOUR = 60`
       - `DAWN_START = 5`, `DAY_START = 7`, `DUSK_START = 18`, `NIGHT_START = 20`
  ...
  <verify>Run a script to instantiate WorldClockService and verify 60 ticks = 1 hour, 1440 ticks = 1 day, and phases match boundaries (5, 7, 18, 20).</verify>
...
<task type="auto">
  ...
  <verify>End player turn in game and check if `world_clock.total_ticks` increments by 1.</verify>
